...
export default function Component({ service }) {
  ...
  const totalSize = sources.reduce((sum, s) => sum + (s.lastSnapshot?.stats?.totalSize ?? 0), 0);
  ...
  const nextTime =
    nextTimes.length > 0 ? new Date(Math.min(...nextTimes.map((time) => new Date(time).getTime()))) : null;
